...
class DiningUnhingedReviewService {
  const DiningUnhingedReviewService({
  ...
  Future<List<DiningUnhingedReview>> fetchReviews() async {
    ...
    return rawVenues
        ...
        .map(
          DiningUnhingedReview.fromJson,
        )
        .where(
          (review) => review.isValid,
        )
        .toList();
  }
  ...
  DiningUnhingedReview? findMatch({
    ...
    if (bestScore < _minimumMatchScore) {
      return null;
    }
  ...
  double _matchScore({
    required DetourVenue venue,
    required DiningUnhingedReview review,
  }) {
    ...
    final locationScore =
        _locationScore(distanceMeters);

    return (nameScore * 0.65) +
        (locationScore * 0.35);
  }
  ...
  double _nameSimilarity(
    ...
    final normalizedFirst =
        _normalizeName(first);

    final normalizedSecond =
        _normalizeName(second);
    ...
    final firstWords =
        normalizedFirst.split(' ').toSet();

    final secondWords =
        normalizedSecond.split(' ').toSet();
...
class DiningUnhingedReviewException
    implements Exception {
  const DiningUnhingedReviewException(
    this.message,
  );
